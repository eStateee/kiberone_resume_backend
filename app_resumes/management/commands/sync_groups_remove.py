...
from app_resumes.models import Group
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            active_crm_ids = {g.get("id") for g in groups_data if g.get("id") is not None}
            ...
            if dry_run:
                ...
                # Удаляем группы. Благодаря on_delete=CASCADE, связанные студенты также будут удалены.
                deleted_count, deletions_detail = groups_to_delete.delete()
                groups_deleted = deletions_detail.get('app_resumes.Group', 0)
                # self.stdout.write(self.style.SUCCESS(
                #     f"Успешно удалено {groups_deleted} групп (всего удалено объектов: {deleted_count}, включая связанных студентов)."
                # ))
                self.stdout.write(self.style.SUCCESS(
